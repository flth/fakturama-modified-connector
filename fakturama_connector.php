...
//die(DIR_FS_CATALOG.DIR_WS_CLASSES . 'Smarty*/Smarty.class.php');
//	$glob_results = glob(DIR_FS_CATALOG.DIR_WS_CLASSES . 'Smarty*/Smarty.class.php');
//	if(count($glob_results) == 1) {
//		foreach($glob_results as $smarty_version) {
//			if(file_exists($smarty_version)) {
...
echo("<webshopexport version=\"" . FAKTURAMA_CONNECTOR_VERSION . "\" xmlns=\"https://www.fakturama.org/\">\n");
...
if (FAKTURAMA_WEBSHOP == 'OSCOMMERCE')
    ...
    echo("shop=\"???\" ");
echo("url=\"" . my_encrypt(HTTPS_CATALOG_SERVER) . "\"");
echo("></webshop>\n");
...
foreach ($orderstosync as $ordertosync) {
    ...
    $notify_comments = '';
    // Is there a comment ?
    if (is_string($orders_status_tosync)) {
        if (strlen($orders_status_tosync) > 1) {
            $notify_comments = substr($orders_status_tosync, 1);
        }
    } else {
        // Handle case when $orders_status_tosync is not a string.
        // You could assign a default string value to $notify_comments here, or give an error message, depending on your needs
        $notify_comments = ''; // or handle the error accordingly
    }
